...
Deno.serve(async req=>{if(req.method==="OPTIONS")return new Response("ok",{headers:cors});if(req.method!=="POST")return json({error:"Method not allowed"},405);try{
 const form=await req.formData();if(clean(form.get("website"),100))return json({ok:true});const name=clean(form.get("name"),80),city=clean(form.get("city"),80),country=clean(form.get("country"),80),body=clean(form.get("body"),40000);const categoryLabels:Record<string,string>={prose:"Prose",poem:"Poème",letter:"Lettre",memory:"Souvenir de Vichy",note:"Note",other:"Autre"};const categories=form.getAll("category").map(value=>clean(value,30)).filter(value=>categoryLabels[value]).slice(0,6);
